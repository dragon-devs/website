      ...
      >
        ...
        <div className="relative h-52 md:h-72 bg-foreground/[0.04] overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center">
            {project.image ? (
              <Image
                src={themedSrc(project.image, isLight)}
                alt={
                  project.title
                    ? `${project.title} — ${project.category} project by dragondevs`
                    : "dragondevs project"
                }
                width={300}
                height={300}
                className={
                  "w-full h-full object-cover transition-transform duration-[1200ms] ease-out group-hover:scale-[1.03]"
                }
              />
            ) : (
              <Boxes
        ...
        <div
          className="absolute inset-x-0 top-0 h-px z-10 bg-[image:var(--forge)]
