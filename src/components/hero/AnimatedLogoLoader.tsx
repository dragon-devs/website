// components/hero/AnimatedLogoLoader.tsx
'use client';
...
interface AnimatedLogoLoaderProps {
	...
	duration?: number;
	/** Optional: Delay between each path animation in milliseconds */
	pathDelay?: number;
	/** Optional: Stroke width */
	strokeWidth?: number;
	/** Optional: Custom stroke color overrides */
	strokeColor?: { dark: string; light: string };
	/** Optional: Whether to loop the animation */
	loop?: boolean;
	...
	className?: string;
	/** Optional: Show a filled version after animation completes */
	showFilled?: boolean;
}
...
export const AnimatedLogoLoader: React.FC<AnimatedLogoLoaderProps> = ({
	...
	const logoStroke = useMemo(
		() => (isDark ? strokeColor.dark : strokeColor.light),
		[isDark, strokeColor]
	);
	...
	useEffect(() => {
		...
		const totalDuration = duration + (svgPaths.length - 1) * pathDelay;
	...
	}, [mounted, duration, pathDelay, svgPaths.length, onComplete, loop, key]);
	...
	return (
		<div className={`relative ${containerPadding} ${className}`}>
			<svg
				key={key}
				...
				<g>
					{svgPaths.map((path, index) => (
						<path
							...
							style={{
								opacity: animationComplete && !showFilled ? 0 : 1,
